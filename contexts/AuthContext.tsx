...
export function AuthProvider({ children }: { children: ReactNode }) {
		...
		try {
			...
			if (Platform.OS === 'ios' && error instanceof Error) {
				if (
					...
					console.error(
						'[iOS] Network/SSL error detected. Keeping existing session if available.',
					);
					if (!user) {
						setUser(null);
					}
	...
	const signInWithEmail = async (email: string, password: string) => {
		try {
			...
			const token = responseData?.session?.token || responseData?.token;
		...
		try {
			...
			if (error instanceof Error) {
				if (error.message.includes('not available yet')) {
					throw new Error(
						'Guest sign-in is temporarily unavailable. Please try again in a moment.',
					);
				} else if (
					error.message.includes('Network request failed') ||
					error.message.includes('SSL') ||
