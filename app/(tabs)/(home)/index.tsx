...
export default function HomeScreen() {
	...
	const loadData = useCallback(async () => {
		...
		try {
			let todayEntries;
			try {
				todayEntries = await authenticatedGet<FoodEntry[]>(
					'/api/food-entries/today',
				);
				console.log(
					'[Home] Today entries loaded:',
					todayEntries?.length || 0,
					'entries',
				);
			} catch (e) {
				console.error('[Home] Failed to load today entries:', e);
			}

			let todayStats;
			try {
				todayStats = await authenticatedGet<TodayStats>(
					'/api/food-entries/stats/today',
				);
				console.log(
					'[Home] Today stats loaded - Calories:',
					todayStats?.totalCalories || 0,
				);
			} catch (e) {
				console.error('[Home] Failed to load today stats:', e);
			}

			let userProfile;
			try {
				userProfile =
					await authenticatedGet<UserProfile>('/api/user/profile');
	...
	const handleAddEntry = async () => {
		...
		console.log('[API] Adding food entry:', {
			foodName,
			calories,
			protein,
			carbs,
		...
		try {
			...
			const newEntry = await authenticatedPost<FoodEntry>(
				'/api/food-entries',
				payload,
			);
		...
		try {
			...
			if (entryToDelete) {
				...
				setStats({
					...
					totalProtein:
						stats.totalProtein - (entryToDelete.protein || 0),
	...
	const calorieProgress = Math.min(
		(stats.totalCalories / calorieGoal) * 100,
		100,
	);
			...
			>
				...
				<View style={dynamicStyles.statsCard}>
					<View style={dynamicStyles.statsHeader}>
						...
						<Text style={dynamicStyles.statsDate}>
					...
					<View style={dynamicStyles.calorieCircle}>
						<Text style={dynamicStyles.calorieNumber}>
							{stats.totalCalories}
						</Text>
						<Text style={dynamicStyles.calorieLabel}>calories</Text>
						<Text style={dynamicStyles.calorieRemaining}>
							{remainingCalories} remaining
						</Text>
						<Text style={dynamicStyles.calorieGoal}>
							Goal: {calorieGoal}
						</Text>
					</View>
					...
					<View style={dynamicStyles.macrosRow}>
						<View style={dynamicStyles.macroItem}>
							<Text
								style={[
									dynamicStyles.macroValue,
									dynamicStyles.macroProtein,
								]}
							>
								{proteinValue}
							</Text>
							<Text style={dynamicStyles.macroLabel}>
								Protein
							</Text>
						</View>
						<View style={dynamicStyles.macroItem}>
						...
						<React.Fragment>
							{mealSections.map((section) => {
								const sectionEntries =
									groupedEntries[section.key];
								if (sectionEntries.length === 0) return null;

								return (
									<View
										key={section.key}
										style={dynamicStyles.mealSection}
									>
										<View
											style={
												dynamicStyles.mealSectionHeader
											}
										>
											<IconSymbol
											...
											<Text
												style={
													dynamicStyles.mealSectionTitle
												}
											>
												{section.label}
											</Text>
										...
										{sectionEntries.map((entry) => {
											...
											const entryProtein = entry.protein
												? `${entry.protein}g`
												: '0g';
											const entryCarbs = entry.carbs
												? `${entry.carbs}g`
												: '0g';
											const entryFat = entry.fat
												? `${entry.fat}g`
												: '0g';
											...
											return (
												...
												>
													...
													<View
														style={
															dynamicStyles.entryHeader
														}
													>
														<View
															style={
																dynamicStyles.entryInfo
															}
														>
															<View
																style={
																	dynamicStyles.entryNameRow
																}
															>
																<Text
																	style={
																		dynamicStyles.entryName
																	}
																>
																	{
																		entry.foodName
													...
													<View
														style={
															dynamicStyles.entryStats
														}
													>
														<View
															style={
																dynamicStyles.entryStat
															}
														>
															<Text
																style={
																	dynamicStyles.entryStatValue
																}
															>
																{entryCalories}
															</Text>
					...
					<View style={dynamicStyles.confirmModal}>
						...
						<View style={dynamicStyles.confirmButtons}>
							...
							>
								<Text
									style={
										dynamicStyles.confirmButtonCancelText
									}
								>
									Cancel
								</Text>
							...
							>
								<Text
									style={
										dynamicStyles.confirmButtonDeleteText
									}
								>
									Delete
								</Text>
			...
			>
				<View style={dynamicStyles.modalOverlay}>
					<View style={dynamicStyles.confirmModal}>
						...
						>
							<Text style={dynamicStyles.confirmButtonDeleteText}>
								OK
							</Text>
						</TouchableOpacity>
					</View>
